...
void Robot_eyes()
{
    ...
    // Animation loop for blinking or moving eyes
    for (int frame = 0; frame < 10; frame++)
    {
        // Clear previous frame
        Paint_Clear(BLACK);

        // Adjust 'a' for vertical size to simulate blinking
        // When 'a' is smaller, the eyes appear more closed
        int a_dynamic = a - frame; // Decrease 'a' to make the eyes "blink"
        if (a_dynamic < 10)
            a_dynamic = 10; // Minimum vertical size to not fully close the eyes

        // Draw the left eye with dynamic 'a'
        Paint_DrawFilledEllipse(left_eye_center_x, eye_center_y, a_dynamic, b, WHITE);

        // Draw the right eye with dynamic 'a'
        Paint_DrawFilledEllipse(right_eye_center_x, eye_center_y, a_dynamic, b, WHITE);

        // Add delay for animation effect
        DEV_Delay_ms(100);

        // You can also change the 'b' value or eye_center_x/y to move the eyes around
    }

    // Ensure to redraw the open eyes after the blinking animation
    Paint_Clear(BLACK);
